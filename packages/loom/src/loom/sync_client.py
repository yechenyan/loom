...
def push_workspaces(workspace_root, server_url: str, workspace: str | None = None, message: str | None = None) -> tuple[WorkspacePushResult, ...]:
    ...
    return tuple(push_single_workspace(workspace_root, resolved_server_url, name, message, _request_json) for name in workspaces if name is not None)
...
def pull_workspaces(workspace_root, server_url: str, workspace: str | None = None) -> tuple[WorkspacePullResult, ...]:
    ...
    return tuple(pull_single_workspace(workspace_root, resolved_server_url, name, _request_json) for name in workspaces)
