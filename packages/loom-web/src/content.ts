...
function sharedConversation(locale: Locale, agent: AgentTab, agentValue: string, packageManager: PackageTab) {
  return {
    en: [
      ...
      { heading: "ask", role: "user", text: "loom ask build a 24-hour German electricity data visualization demo" },
      { role: "assistant", text: "I will inspect the Loom data cards, locate the relevant raw files, and answer with sources." },
      { heading: "review", role: "user", text: "Please verify the generated data cards and compare them with the required raw files." },
      { role: "assistant", text: "I will check the cards, summaries, and required raw files, then fix issues before continuing." },
    ...
    de: [
      ...
      { heading: "review", role: "user", text: "Bitte pruefe die gerade erzeugten Datenkarten und vergleiche sie mit den noetigen Rohdateien." },
      { role: "assistant", text: "Ich pruefe Karten, Zusammenfassungen und noetige Rohdateien und korrigiere Probleme vor dem Weiterarbeiten." },
      { heading: "share", role: "user", text: "loom push" },
    ...
    zh: [
      ...
      { heading: "review", role: "user", text: "请检查刚生成的数据卡片是否准确，并对照必要的原始文件。" },
      { role: "assistant", text: "我会核对数据卡片、摘要和必要原始文件，发现问题会先修正再继续。" },
      { heading: "share", role: "user", text: "loom push" },
      { role: "assistant", text: "The push succeeded." },
    ],
  }[locale];
